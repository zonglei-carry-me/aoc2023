...
void solve(int src, int dst)
{
    ...
    for(int i=0;i<3;i++) {
        ...
        for(auto b: edges){
            ...
            for(auto e: edges){
                if (e!=b && block.find(e)==block.end())
                    F.add_edge(e.first, e.second, 1);
            }
            if (F.max_flow()== cuts-sz(block)-1) {
                block.insert(b);
                break;
            }
        }
    ...
    /*
    block.insert({217,216});
    block.insert({597,886});
    block.insert({1373,755});*/
...
int main() {
    ...
    while(1){
        ...
        for (auto e: edges) F.add_edge(e.first, e.second, 1);
